function prc_PredictionBias_ehgf_binary_combObs_plotTraj(r)
...
%
%
% MODIFICATION FOR SPIRL STUDY: allows for plotting response data from two
% different modalities in the same plot.
% 
%
% --------------------------------------------------------------------------------------------------
...
if (ploty == true) && ~isempty(find(strcmp(fieldnames(r),'y'))) && ~isempty(r.y)
    ...
    title(['Binary response y (orange), input u (green), learning rate (fine black), and posterior expectation of input s(\mu_2) ', ...
           '(red) for \rho=', num2str(r.p_prc.rho(2:end)), ', \kappa=', ...
           num2str(r.p_prc.ka(2:end)), ', \omega=', num2str(r.p_prc.om(2:end))], ...
      'FontWeight', 'bold');
    ylabel('y_{bin}, u, s(\mu_2)');
...
plot(r.y(:,2), 'b')
    hold all;
    plot(r.optim.yhat(:,2), 'r')
    xlim([0 160])
    title(['Cont. Response y (blue) and posterior expectation of cont. ', ...
           'response y_{hat} (red) for obs transp param vector ', num2str(r.p_obs.ptrans)], ...
      'FontWeight', 'bold');
    ylabel('$log(y_{rt}), log(\hat{y}_{rt})$', 'Interpreter', 'Latex')
